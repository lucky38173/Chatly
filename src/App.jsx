...
// ═══════════════════════════════════════════════════════════════
// 🔧 REPLACE THESE 2 VALUES with your Supabase project details
...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .modal-tip span{color:var(--accent2);font-weight:500}
  ...
  .mact{display:flex;gap:2px;opacity:0;transition:opacity .15s;align-self:center;margin-bottom:18px}
  .mw:hover .mact{opacity:1}
  .mact-btn{width:24px;height:24px;border-radius:6px;background:var(--surface2);border:1px solid var(--border);color:var(--text3);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .15s;padding:0;font-size:12px}
  .mact-btn:hover{background:rgba(239,68,68,0.15);color:var(--red);border-color:rgba(239,68,68,0.3)}
  ...
  .sbtn{width:36px;height:36px;border-radius:11px;background:linear-gradient(135deg,var(--accent),#4f46e5);border:none;color:white;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .2s;box-shadow:0 2px 10px rgba(99,102,241,0.4);flex-shrink:0;font-size:16px}
...
const IcoBell    = () => <Ico d={["M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9","M13.73 21a2 2 0 0 1-3.46 0"]} size={16}/>;
...
function UsernameSetup({userId, onDone}) {
  ...
  const hints = {idle:{cls:"info",msg:"3–20 characters. Letters, numbers, underscore only."},checking:{cls:"info",msg:"Checking…"},invalid:{cls:"err",msg:"3–20 chars, only a–z 0–9 _"},taken:{cls:"err",msg:`@${val} is already taken.`},ok:{cls:"ok",msg:`@${val} is available ✓`}};
  ...
  return (
    ...
    <div className="auth"><div className="auth-bg"/><div className="auth-grid"/>
      <div className="auth-card">
        ...
        <p className="auth-sub">Others will find and add you by @username. Choose wisely!</p>
        <div className="uname-input-wrap">
          ...
          <input className={`uname-input${status==="ok"?" ok":status==="taken"||status==="invalid"?" err":""}`}
            value={val} onChange={onChange} onKeyDown={e=>e.key==="Enter"&&confirm()} placeholder="your_username" maxLength={20} autoFocus/>
        </div>
      ...
      <div className="modal">
        ...
        <div className="modal-body">
          <div className="modal-search">
            ...
            <input ref={inputRef} value={query} onChange={e=>search(e.target.value)} onKeyDown={e=>e.key==="Escape"&&onClose()} placeholder="username"/>
          </div>
...
export default function App() {
  ...
  const [convs, setConvs]       = useState([]);  // [{id, contact:{...}, messages:[...], lastMsg}]
  ...
  const conv = activeId ? convs.find(c=>c.id===activeId) : null;
  ...
  const loadConversations = async () => {
    ...
    const {data:parts} = await supabase
      .from("participants").select("conversation_id").eq("user_id", myId);
    ...
    (allParts||[]).forEach(p => {
      ...
      const msgs = (lastMsgs||[]).filter(m=>m.conversation_id===p.conversation_id)
        .sort((a,b)=>new Date(a.created_at)-new Date(b.created_at));
      convMap[p.conversation_id] = {
        id: p.conversation_id,
        contact: { id:contact.id, name:contact.full_name, username:contact.username, initials:contact.avatar_initials||genInitials(contact.full_name), color:contact.color||randColor(), status:contact.status||"offline", avatar_url:contact.avatar_url },
        messages: msgs.map(m=>({
  ...
  useEffect(() => {
    ...
    realtimeSub.current = supabase
      .channel("messages_rt")
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"messages",filter:`conversation_id=in.(${convIds.join(",")})`},
        (payload) => {
          const m = payload.new;
          setConvs(prev => prev.map(c => {
            if (c.id !== m.conversation_id) return c;
            const newMsg = { id:m.id, from:m.sender_id, text:m.content, isAi:m.is_ai, time:fmtTime(m.created_at), timestamp:new Date(m.created_at).getTime(), read:false, isNew:true };
  ...
  }, [profile?.id, convs.map(c=>c.id).join(",")]);
  ...
  const send = useCallback(async () => {
    ...
    await sb.from("messages").insert({conversation_id:activeId, sender_id:profile.id, content:text});
  }, [draft, activeId, profile]);

  const onKey = useCallback((e) => { if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send();} if(e.key==="Escape")setEmoji(false); }, [send]);
  const onTA  = (e) => { setDraft(e.target.value); e.target.style.height="auto"; e.target.style.height=Math.min(e.target.scrollHeight,120)+"px"; };
  const insertEmoji = (em) => {
    const ta=taRef.current; if(!ta){setDraft(d=>d+em);return;}
    const s=ta.selectionStart??draft.length; const e=ta.selectionEnd??draft.length;
    setDraft(draft.slice(0,s)+em+draft.slice(e));
    requestAnimationFrame(()=>{ if(!taRef.current)return; const p=s+[...em].length; taRef.current.selectionStart=p; taRef.current.selectionEnd=p; taRef.current.style.height="auto"; taRef.current.style.height=Math.min(taRef.current.scrollHeight,120)+"px"; taRef.current.focus(); });
  };
  ...
  const useReply = useCallback(() => {
    if(!aiResult)return; const t=aiResult; setAiResult(null);setAiTask(null);setAiError(null);setDraft(t);
  ...
  const addContact = useCallback(async (user) => {
    if (convs.find(c=>c.contact.id===user.id)) { setActiveId(convs.find(c=>c.contact.id===user.id).id); setAddOpen(false); return; }
  ...
  const filteredConvs = useMemo(() => {
    ...
    return convs.filter(c => !q || c.contact.name?.toLowerCase().includes(q) || c.contact.username?.toLowerCase().includes(q) || c.messages[c.messages.length-1]?.text?.toLowerCase().includes(q))
      .sort((a,b)=>(b.messages[b.messages.length-1]?.timestamp||0)-(a.messages[a.messages.length-1]?.timestamp||0));
  }, [convs, search]);

  const cc = draft.length;
  const showCC = cc > MAX_CHARS * 0.7;
  const myId = profile?.id || session?.user?.id;
  ...
  if (!session) return (
    ...
    <div className="auth"><div className="auth-bg"/><div className="auth-grid"/>
      <div className="auth-card">
        ...
        <p className="auth-sub">Real-time messaging powered by Claude AI. Sign in to get started.</p>
  ...
  if (!profile || !profile.username) {
    return <UsernameSetup userId={session.user.id} onDone={async (username) => {
      ...
      <div className={`sidebar${activeId?" hide":""}`}>
        <div className="sb-head">
          <div style={{width:36,height:36,borderRadius:10,background:(profile.color||"#6366f1")+"22",border:`1px solid ${profile.color||"#6366f1"}44`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:600,color:profile.color||"#6366f1",flexShrink:0,overflow:"hidden"}}>
            {profile.avatar_url ? <img src={profile.avatar_url} style={{width:"100%",height:"100%",objectFit:"cover",borderRadius:10}} onError={e=>e.target.style.display="none"}/> : (profile.avatar_initials||genInitials(profile.full_name))}
          </div>
            ...
            return (
              <div key={c.id} className={`ci${activeId===c.id?" on":""}`} onClick={()=>setActiveId(c.id)}>
                ...
                <div className="ci-info">
                  <div className="ci-name">{c.contact.name}</div>
